...
namespace backend_dot_net_core.Core.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<GeneralServiceResponseDto> RegisterAsync(RegisterDto registerDto)
        {
            ...
            await _logService.SaveNewLog(newUser.UserName, "Registedered to the Website");
        ...
        public async Task<LoginServiceResponseDto?> LoginAsync(LoginDtos loginDto)
        {
            // ind user with username
            ...
            else
            {
                ...
                if (UserRoles.Any(q => q.Equals(StaticUserRoles.OWNER)))
                    return new GeneralServiceResponseDto()
                    {
                        IsSucceed = false,
                        StatusCode = 403,
                        Message = "you are not allowed yo change role of this user"
        ...
        public async Task<LoginServiceResponseDto?> MeAsync(MeDto meDto)
        {
            ...
            var user = await _userManager.FindByEmailAsync(decodedUserName);
            if(user is null)
            { return null; }
...
// generate JWTTokenAsync
        ...
        {
            //Instead of this, You can use Automapper packages,
            //But i don't want it in this project 
